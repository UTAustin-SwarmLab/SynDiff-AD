            ...
            for phase in phases:
                ...
                for data in dataloaders[phase]:
                    ...
                    with torch.set_grad_enabled(phase == "train"):
                        ...
                        pred_wp = model(scene, history=history)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--device_name", type=str, default="pulkit")
    parser.add_argument("--split", type = str, default = "0.2")
    ...
    SCRATCH_DIR = "/home/pulkit/datasets_xplane/".replace("pulkit", device_name)+split+"/trained_models_wp"
    device_name = parser.parse_args().device_name
    ...
    for condition_list in experiment_list:
        ...
        vae_save_path = (
            f"/home/pulkit/datasets_xplane/"+split+"/trained_models_svae/" f"taxinet_VAE_Z{Z_DIM}_all_data/{condition_str}/best_model_svae.pt"
        )
        if not os.path.exists(vae_save_path):
            vae_save_path = (
            f"/home/pulkit/datasets_xplane/"+split+"/trained_models_svae/" f"taxinet_VAE_Z{Z_DIM}_all_data/{condition_str}/best_model_95.pt"
            )
        # vae_save_path = "/home/pulkit/datasets_xplane/""/best_model_svae.pt"
        ...
        def load_trainedModel_cuda(model_path,device=torch.device('cuda')):
            vae_model = SoftIntroVAE(
                cdim=3,
                zdim=Z_DIM,
                channels=[64, 128, 256, 512, 512],
        ...
        if vae_save_path is not None:
            ...
            vae_model = load_trainedModel_cuda(vae_save_path)
        ...
        model = PerceptionWaypointModel(
            vae_model,
            input_size=Z_DIM + 2*HISTORY_LEN,
        ...
        data_dir = "/home/pulkit/datasets_xplane/".replace("pulkit", device_name) + split + "/xplane_dataset/"
        ...
        if not os.path.isdir("/home/pulkit/datasets_xplane/".replace("pulkit", device_name)+split+"/taxinet_wp"):
            os.mkdir("/home/pulkit/datasets_xplane/".replace("pulkit", device_name)+split+"/taxinet_wp")
        results_dir = "/home/pulkit/datasets_xplane/".replace("pulkit", device_name)+split+"/taxinet_wp"
