...
def get_data_from_dir(
    ...
    image_dir = "/home/pulkit/triplets/".replace("pulkit", DEVICE_NAME)
    image_list = [x for x in os.listdir(image_dir) if x.endswith(".png")]
    print("Length of image list: ", len(image_list))
    dataloader_dir = "/home/pulkit/CSV_FILES/".replace("pulkit", DEVICE_NAME)
    with open(os.path.join(dataloader_dir, "AFT_waypoint_data.pickle"), "rb") as handle:
...
class TaxiNetDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        images, waypoint_dict, self.scaler = get_data_from_dir(dataloader_dir, history_len, train_test_split, scaler)
        ...
        self.waypoint_dict.update(waypoint_dict)
                # with open("/home/pulkit/CSV_FILES/AFT_waypoint_data.pickle".replace("pulkit", DEVICE_NAME), "rb") as handle:
                #     waypoint_dict = pickle.load(handle)
                # self.waypoint_dict = waypoint_dict
                # image_list = [x for x in os.listdir("/home/pulkit/imgs/".replace("pulkit", DEVICE_NAME)) if x.endswith(".jpg")]
                # self.all_images = list(set(image_list).intersection(set(list(waypoint_dict.keys()))))
...
def taxinet_prepare_dataloader(
    ...
    print(history_len)
    taxinet_dataset = TaxiNetDataset(data_dir, train_test_split, history_len, train_scaler, exper_types)
